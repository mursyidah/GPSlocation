package fileencrypt.geoencrypt.admin.gpslocation.Database;
...
public class DatabaseAdapter {
    ...
    private static final String STORE_TABLE = "location";
    //Table unique id
    ...
    public long createUser2(String latitude, String longitude) {
        ContentValues initialValues = createStoreTable(latitude, longitude);
        return database.insert(STORE_TABLE, null, initialValues);
    }
    ...
    public Cursor fetchPass(String password) {
        Cursor myCursor = database.query(LOGIN_TABLE,
    ...
    public Cursor fetchUser2(String latitude, String longitude) {
        Cursor myCursor = database.query(STORE_TABLE,
                new String[]{COL_ID2, COL_LATITUDE, COL_LONGITUDE},
                COL_LATITUDE + "='" + latitude + "' AND " +
                        COL_LONGITUDE + "='" + longitude + "'", null, null, null, null);
    ...
    public Cursor fetchUserById2(long rowId) throws SQLException {
        Cursor myCursor = database.query(STORE_TABLE,
                new String[]{COL_ID2, COL_LATITUDE, COL_LONGITUDE},
                COL_ID2 + "=" + rowId, null, null, null, null);
        if (myCursor != null) {
            myCursor.moveToFirst();
        }
        return myCursor;
    ...
    private ContentValues createStoreTable(String latitude, String longitude) {
        ContentValues values = new ContentValues();
